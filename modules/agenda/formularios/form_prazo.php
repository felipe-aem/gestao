...
// Buscar etiquetas ativas do tipo prazo ou geral
$sql_etiquetas = "SELECT id, nome, cor, icone 
                  FROM etiquetas 
                  WHERE ativo = 1 AND tipo IN ('prazo', 'geral')
                  ORDER BY nome";
...
<div class="form-inline-container">
    <div class="form-inline-header">
        ...
        <button type="button" class="btn-close-form" onclick="fecharFormularioPrazo()">
            <i class="fas fa-times"></i>
        </button>
    </div>
    
    <form id="formPrazo" method="POST">
        ...
        <div class="form-actions">
            <button type="button" class="btn btn-secondary" onclick="fecharFormularioPrazo()">
                <i class="fas fa-times"></i>
                Cancelar
            </button>
...
(function() {
    'use strict';
    console.log('🚀 [INIT] Iniciando form_prazo.php');
    
    function init() {
        console.log('🎯 [INIT] Configurando formulário de prazo');
                            ...
                            if (processoSelecionado) {
                                processoSelecionado.innerHTML = `
                                    <div class="processo-selecionado">
                                        ...
                                        <button type="button" class="btn-remover-processo" onclick="removerProcessoPrazo()">
                                            <i class="fas fa-times"></i>
                                        </button>
                                    </div>
                                `;
        ...
        window.removerProcessoPrazo = function() {
            const processoId = document.getElementById('processo_id');
            const processoSelecionado = document.getElementById('processoSelecionado');
        ...
        // Form prazo
        const formPrazo = document.getElementById('formPrazo');
        if (formPrazo) {
            formPrazo.addEventListener('submit', async function(e) {
                e.preventDefault();
                console.log('💾 [PRAZO] Salvando...');
                ...
                try {
                    const formData = new FormData(this);
                    const response = await fetch('/modules/agenda/formularios/salvar_prazo.php', {
                        method: 'POST',
                        body: formData
                    });
