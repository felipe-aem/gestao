...
// Verificar permissão
$usuarios_especiais = [28]; // IDs dos usuários com permissão especial
...
// Buscar histórico DETALHADO (incluindo mudanças de fase E interações)
try {
    $sql_hist = "
        SELECT 
            h.id,
            ...
            h.observacao,
            h.usuario_id,
            h.data_movimento as data_registro,
            h.campo_alterado,
            h.valor_anterior,
            h.valor_novo,
            h.tipo_acao,
        ...
        SELECT 
            i.id,
            i.prospeccao_id,
            NULL as fase_anterior,
            i.tipo as fase_nova,
            NULL as valor_informado,
            i.descricao as observacao,
            i.usuario_id,
            i.data_interacao as data_registro,
            NULL as campo_alterado,
            NULL as valor_anterior,
            NULL as valor_novo,
            NULL as tipo_acao,
            u.nome as usuario_nome,
            'interacao' as tipo_historico
    ...
    $stmt_hist = executeQuery($sql_hist, [$id, $id]);
    $historico_completo = $stmt_hist->fetchAll();
    
    error_log("DEBUG ADVOCACIA - Histórico completo count: " . count($historico_completo) . " para prospecto ID: " . $id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adicionar_interacao'])) {
    try {
        ...
        if ($result) {
            header('Location: visualizar_advocacia.php?id=' . $id . '&interacao=sucesso');
            exit;
        } else {
...
<div class="view-container">
    ...
    <div class="view-header">
        ...
        <div class="view-header-actions">
            <?php if ($pode_editar): ?>
                <a href="editar_advocacia.php?id=<?= $id ?>" class="btn btn-primary">
                    ✏️ Editar
                </a>
            <?php endif; ?>
    ...
    <div class="info-section">
        ...
        <div class="info-grid">
            ...
            <div class="info-field">
                <div class="info-label">🏛️ Núcleo</div>
                <div class="info-value"><?= htmlspecialchars($prospecto['nucleo_nome']) ?></div>
            </div>
            ...
            <?php if (!empty($prospecto['indicacao'])): ?>
            <div class="info-item">
                <div class="info-label">👥 Indicação</div>
                <div class="info-value"><?= htmlspecialchars($prospecto['indicacao']) ?></div>
            </div>
            <?php endif; ?>
                    ...
                    <div class="timeline-item <?= $item['tipo_historico'] ?>">
                        <div class="timeline-content">
                            <div class="timeline-header">
                                <div class="timeline-title">
                                    <?php if ($item['tipo_historico'] === 'mudanca_fase'): ?>
                                        <?php 
                                        // Verificar se é mudança de fase ou edição de campo
                                        $tipo_acao = $item['tipo_acao'] ?? 'mudanca_fase';
                                        ?>
                                        
                                        <?php if ($tipo_acao === 'edicao_campo'): ?>
                                            ✏️ Editou: <?= htmlspecialchars($item['campo_alterado']) ?>
                                        <?php else: ?>
                                            🔄 Mudança de Fase
                                            ...
                                            <?php endif; ?>
                                        <?php endif; ?>
                                    <?php elseif ($item['tipo_historico'] === 'interacao'): ?>
                                        <?php
                                        $icone_tipo = [
                                            'ligacao' => '📞',
                                            'email' => '📧',
                                        ...
                                        $icone = $icone_tipo[$item['fase_nova']] ?? '💬';
                                        $label_tipo = ucfirst($item['fase_nova']);
                                        ?>
                                        <?= $icone ?> <?= $label_tipo ?>
                                    <?php else: ?>
                            ...
                            <div class="timeline-description">
                                ...
                                <?php if (isset($item['tipo_acao']) && $item['tipo_acao'] === 'edicao_campo'): ?>
                                    <!-- Exibir mudança de campo detalhada -->
                                    <div style="margin-top: 10px; padding: 12px; background: white; border-radius: 6px; border-left: 3px solid #667eea;">
                                        <div style="display: grid; grid-template-columns: 1fr auto 1fr; gap: 10px; align-items: center;">
                                            <div style="text-align: right;">
                                                <small style="color: #95a5a6; display: block; margin-bottom: 4px; font-size: 11px; text-transform: uppercase;">Antes:</small>
                                                <span style="color: #e74c3c; font-weight: 600;"><?= htmlspecialchars($item['valor_anterior']) ?></span>
                                            </div>
                                            <div style="color: #667eea; font-size: 18px; font-weight: bold;">→</div>
                                            <div style="text-align: left;">
                                                <small style="color: #95a5a6; display: block; margin-bottom: 4px; font-size: 11px; text-transform: uppercase;">Depois:</small>
                                                <span style="color: #27ae60; font-weight: 600;"><?= htmlspecialchars($item['valor_novo']) ?></span>
                                            </div>
                                        </div>
                                    </div>
                                <?php else: ?>
                                    <!-- Exibir mudança de fase normal -->
...
echo renderLayout('Visualizar Prospecto - ADVOCACIA', $conteudo, 'prospeccao');
?>