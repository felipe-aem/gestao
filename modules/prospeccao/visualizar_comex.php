...
try {
    ...
    if (!$prospecto) {
        header('Location: comex.php?erro=nao_encontrado');
        exit;
    }
} catch (Exception $e) {
    header('Location: comex.php?erro=busca');
    exit;
}
...
try {
    ...
    error_log("DEBUG COMEX - Histórico completo count: " . count($historico_completo) . " para prospecto ID: " . $id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['adicionar_interacao'])) {
    try {
        ...
        if ($result) {
            header('Location: visualizar_comex.php?id=' . $id . '&interacao=sucesso');
            exit;
        } else {
...
<div class="view-container">
    ...
    <div class="view-header">
        ...
        <div class="view-header-actions">
            <?php if ($pode_editar): ?>
                <a href="editar_comex.php?id=<?= $id ?>" class="btn btn-primary">
                    ✏️ Editar
                </a>
            <?php endif; ?>
...
echo renderLayout('Visualizar Prospecto - COMEX', $conteudo, 'prospeccao');
?>